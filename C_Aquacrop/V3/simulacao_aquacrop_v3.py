...
Saídas:
  - dataset_cold_start_v3.csv
  - dataset_cold_start_completo_v3.csv
  - cenario1_otimo_v3.csv / cenario2_deficit_v3.csv / cenario3_excesso_v3.csv
  - graficos_v3/ (distribuicao_classes, correlacao, histograma_IrrDay)
...
CENARIOS = [
    (1, 'cenario1_otimo_v3',   65, 'Irrigação Ótima  (NetIrrSMT=65%)'),
    (2, 'cenario2_deficit_v3', 30, 'Déficit Hídrico  (NetIrrSMT=30%)'),
    (3, 'cenario3_excesso_v3', 90, 'Excesso Hídrico  (NetIrrSMT=90%)'),
]
...
def calcular_limiares_percentis(df_todos):
    """
    Calcula limiares das classes 1-4 usando percentis P20/P40/P60/P80
    dos valores de IrrDay > 0 no dataset combinado.
    Retorna dict com limiares e flag se usou 3 ou 5 classes.
    """
    irr_positivo = df_todos[df_todos['irr_mm'] > 0]['irr_mm']
    n_positivos  = len(irr_positivo)
    n_total      = len(df_todos)

    print(f"\n{'='*60}")
    print("🔍 ANÁLISE DO IrrDay BRUTO (Problema 1)")
    print(f"   Total amostras     : {n_total}")
    print(f"   IrrDay = 0         : {(df_todos['irr_mm']==0).sum()} ({(df_todos['irr_mm']==0).mean()*100:.1f}%)")
    print(f"   IrrDay > 0         : {n_positivos} ({n_positivos/n_total*100:.1f}%)")
    print(f"\n   Distribuição IrrDay > 0:")
    print(f"   Mín    : {irr_positivo.min():.2f} mm")
    print(f"   P20    : {irr_positivo.quantile(0.20):.2f} mm")
    print(f"   P40    : {irr_positivo.quantile(0.40):.2f} mm")
    print(f"   Mediana: {irr_positivo.quantile(0.50):.2f} mm")
    print(f"   P60    : {irr_positivo.quantile(0.60):.2f} mm")
    print(f"   P80    : {irr_positivo.quantile(0.80):.2f} mm")
    print(f"   Máx    : {irr_positivo.max():.2f} mm")

    p20 = irr_positivo.quantile(0.20)
    p40 = irr_positivo.quantile(0.40)
    p60 = irr_positivo.quantile(0.60)
    p80 = irr_positivo.quantile(0.80)
    mediana = irr_positivo.quantile(0.50)

    print(f"\n   Por cenário:")
    for nome_c in df_todos['cenario'].unique():
        sub = df_todos[df_todos['cenario']==nome_c]
        irr_pos = sub[sub['irr_mm'] > 0]['irr_mm']
        label = nome_c.replace('cenario', 'C').replace('_v3', '')
        if len(irr_pos) > 0:
            print(f"   {label}: n={len(irr_pos)} | "
                  f"min={irr_pos.min():.1f} | med={irr_pos.median():.1f} | "
                  f"max={irr_pos.max():.1f} mm")
        else:
            print(f"   {label}: sem irrigações positivas")

    # Testar 5 classes com percentis
    def rotular_5(x, p20, p40, p60, p80):
        if x == 0.0:       return 0
        elif x <= p20:     return 1
        elif x <= p40:     return 2
        elif x <= p60:     return 3
        elif x <= p80:     return 4
        else:              return 4  # agrupa >P80 na classe 4

    df_todos['cls_test5'] = df_todos['irr_mm'].apply(
        lambda x: rotular_5(x, p20, p40, p60, p80))
    dist5 = df_todos['cls_test5'].value_counts().sort_index()
    min_pct5 = dist5.min() / n_total

    print(f"\n   Distribuição com 5 classes (percentis):")
    for k, v in dist5.items():
        print(f"   C{k}: {v} ({v/n_total*100:.1f}%)")

    if min_pct5 >= 0.05:
        print(f"\n   ✅ 5 classes aceitas (menor classe: {min_pct5*100:.1f}% >= 5%)")
        n_classes = 5
        limiares = {'p20': p20, 'p40': p40, 'p60': p60, 'p80': p80}
    else:
        # Fallback 3 classes
        print(f"\n   ⚠️  Menor classe: {min_pct5*100:.1f}% < 5% → fallback 3 classes")
        df_todos['cls_test3'] = df_todos['irr_mm'].apply(
            lambda x: 0 if x == 0 else (1 if x <= mediana else 2))
        dist3 = df_todos['cls_test3'].value_counts().sort_index()
        print(f"   Distribuição com 3 classes:")
        for k, v in dist3.items():
            print(f"   C{k}: {v} ({v/n_total*100:.1f}%)")
        n_classes = 3
        limiares = {'mediana': mediana}

    df_todos.drop(columns=[c for c in ['cls_test5','cls_test3'] if c in df_todos.columns],
                  inplace=True)
    return n_classes, limiares


def rotular_classe_v3(irr_mm, n_classes, limiares):
    if irr_mm == 0.0:
        return 0
    if n_classes == 5:
        p20, p40, p60, p80 = limiares['p20'], limiares['p40'], limiares['p60'], limiares['p80']
        if irr_mm <= p20:   return 1
        elif irr_mm <= p40: return 2
        elif irr_mm <= p60: return 3
        else:               return 4
    else:  # 3 classes
        return 1 if irr_mm <= limiares['mediana'] else 2
...
def gerar_graficos(df_final, df_irr_bruto, feature_chuva, n_classes):
    ...
    fig.suptitle(f'Distribuição de Classes por Cenário (v3 — {n_classes} classes, percentis)',
                 fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(f'{DIR_GRAFICOS}/distribuicao_classes_v3.png', dpi=130)
    plt.close()
    print(f"   📈 Salvo: {DIR_GRAFICOS}/distribuicao_classes_v3.png")
    ...
    ax.set_title('Matriz de Correlação — Dataset v3', fontweight='bold')
    plt.tight_layout()
    plt.savefig(f'{DIR_GRAFICOS}/correlacao_features_v3.png', dpi=130)
    plt.close()
    print(f"   📈 Salvo: {DIR_GRAFICOS}/correlacao_features_v3.png")
    ...
    fig.suptitle('Comparação de Distribuição de Classes — v2 (limiares fixos) vs v3 (percentis)',
    ...
    for col_idx, (cid, nome, _, _desc) in enumerate(CENARIOS):
        ...
        axes[1][col_idx].set_title(f"C{cid}: {label} — v3 (percentis)")
    ...
    fig.suptitle('Comparação de Correlação — v2 (limiares fixos) vs v3 (percentis)',
                 fontweight='bold')

    for ax, cols, titulo in [(ax1, cols_v2_disp, 'v2 — limiares fixos'),
                              (ax2, cols_v3_disp, f'v3 — percentis ({n_classes} classes)')]:
...
def gerar_relatorio(n_classes, limiares, feature_chuva, corrs_chuva,
                    n_cravados, n_total, dist_final, dist_por_cenario,
                    theta_33, theta_1500, A, B):
    linhas = []
    linhas.append("# Relatório de Correções — Dataset Cold Start v3\n")
    ...
    linhas.append("## PROBLEMA 1 — Rotulagem por Percentis Reais do IrrDay\n")
    linhas.append(f"**Número de classes adotado:** {n_classes}\n")
    if n_classes == 5:
        linhas.append("**Limiares adotados (IrrDay > 0):**\n")
        linhas.append(f"- Classe 0: IrrDay = 0,0 mm (sem irrigação)\n")
        linhas.append(f"- Classe 1: 0 < IrrDay ≤ {limiares['p20']:.2f} mm (P20)\n")
        linhas.append(f"- Classe 2: {limiares['p20']:.2f} < IrrDay ≤ {limiares['p40']:.2f} mm (P40)\n")
        linhas.append(f"- Classe 3: {limiares['p40']:.2f} < IrrDay ≤ {limiares['p60']:.2f} mm (P60)\n")
        linhas.append(f"- Classe 4: IrrDay > {limiares['p60']:.2f} mm (acima de P60)\n")
    else:
        linhas.append("**Fallback para 3 classes aplicado** (distribuição com 5 classes muito desbalanceada):\n")
        linhas.append(f"- Classe 0: IrrDay = 0,0 mm\n")
        linhas.append(f"- Classe 1: 0 < IrrDay ≤ {limiares['mediana']:.2f} mm (mediana)\n")
        linhas.append(f"- Classe 2: IrrDay > {limiares['mediana']:.2f} mm\n")
    ...
    linhas.append("- `dataset_cold_start_v3.csv` — 5 colunas para ALMMo-0\n")
    linhas.append("- `dataset_cold_start_completo_v3.csv` — com metadados de cenário/ano\n")
    linhas.append("- `graficos_v3/histograma_IrrDay.png`\n")
    linhas.append("- `graficos_v3/distribuicao_classes_v3.png`\n")
    linhas.append("- `graficos_v3/correlacao_features_v3.png`\n")
...
def main():
    print("\n" + "="*60)
    print("🌿 SIMULAÇÃO AQUACROP v3 — Cold Start ALMMo-0")
    ...
    df_todos['classe_irrigacao'] = df_todos['irr_mm'].apply(
        lambda x: rotular_classe_v3(x, n_classes, limiares))
    ...
    df_todos_out[colunas_almmo].to_csv('dataset_cold_start_v3.csv', index=False)
    df_todos_out[colunas_almmo + ['irr_mm', 'ano', 'cenario']].to_csv(
        'dataset_cold_start_completo_v3.csv', index=False)
    ...
    # PASSO 9 — Relatório
    gerar_relatorio(n_classes, limiares, feature_chuva, corrs_chuva,
                    n_cravados, n_total, dist_final, dist_por_cenario,
                    theta_33, theta_1500, A, B)

    print(f"\n{'='*60}")
    print(f"✅ dataset_cold_start_v3.csv → {n_total} linhas | {len(colunas_almmo)} colunas")
    print(f"   Colunas: {colunas_almmo}")
    print(f"   Classes: {n_classes} | Limiares: por percentis reais do IrrDay")
    print(f"   Chuva: {feature_chuva}")
    print(f"\n→ Próximo passo: dataset_cold_start_v3.csv → ALMMo-0 cold_start()")
    print("="*60)
