...
VERSÃO 6 — NetIrrSMT recalibrado para faixa agronômica correta (40-80 kPa)
=============================================================================
...
Saídas:
  - dataset_cold_start_v6.csv
  - dataset_cold_start_completo_v6.csv
  - cenario1_otimo_v6.csv / cenario2_deficit_v6.csv / cenario3_excesso_v6.csv
  - graficos_v6/ (distribuicao_classes, correlacao, histograma_IrrDay)
...
CENARIOS = [
    # v6: NetIrrSMT recalibrado para faixa agronômica correta do tomate (40-80 kPa)
    # Conversão via S&R: NetIrrSMT=X% → θ = θ_PM + X/100*(θ_CC-θ_PM) → tensão = A*θ^(-B)
    # NetIrrSMT=79% → tensão ~60 kPa (gatilho FAO para tomate)
    # NetIrrSMT=69% → tensão ~80 kPa (stress moderado — déficit controlado)
    # NetIrrSMT=93% → tensão ~40 kPa (zona ótima — irrigação abundante)
    (1, 'cenario1_otimo_v6',   79, 'Irrigação Ótima  (NetIrrSMT=79% ≈ 60 kPa)'),
    (2, 'cenario2_deficit_v6', 69, 'Déficit Hídrico  (NetIrrSMT=69% ≈ 80 kPa)'),
    (3, 'cenario3_excesso_v6', 93, 'Excesso Hídrico  (NetIrrSMT=93% ≈ 40 kPa)'),
]
...
def calcular_limiares_percentis(df_todos):
    ...
    print(f"\n{'='*60}")
    print("🔍 ANÁLISE DO IrrDay BRUTO (v6 — 3 classes, mediana do ótimo)")
...
def gerar_graficos(df_final, df_irr_bruto, feature_chuva, n_classes):
    ...
    plt.tight_layout()
    plt.savefig(f'{DIR_GRAFICOS}/distribuicao_classes_v6.png', dpi=130)
    plt.close()
    print(f"   📈 Salvo: {DIR_GRAFICOS}/distribuicao_classes_v6.png")
    ...
    ax.set_title('Matriz de Correlação — Dataset v6', fontweight='bold')
    plt.tight_layout()
    plt.savefig(f'{DIR_GRAFICOS}/correlacao_features_v6.png', dpi=130)
    plt.close()
    print(f"   📈 Salvo: {DIR_GRAFICOS}/correlacao_features_v6.png")
    ...
    fig.suptitle('Comparação de Distribuição de Classes — v2 (limiares fixos) vs v6 (3 classes, mediana ótimo)',
    ...
    fig.suptitle('Comparação de Correlação — v2 (limiares fixos) vs v6 (3 classes, mediana ótimo)',
                 fontweight='bold')

    for ax, cols, titulo in [(ax1, cols_v2_disp, 'v2 — limiares fixos'),
                              (ax2, cols_v3_disp, f'v6 — 3 classes, mediana ótimo (5 classes)')]:
...
def gerar_relatorio(limiares, feature_chuva, corrs_chuva,
                    ...
                    theta_33, theta_1500, A, B):
    linhas = []
    linhas.append("# Relatório de Correções — Dataset Cold Start v6\n")
    ...
    linhas.append("- `dataset_cold_start_v6.csv` — 5 colunas para ALMMo-0\n")
    linhas.append("- `dataset_cold_start_completo_v6.csv` — com metadados de cenário/ano\n")
    linhas.append("- `graficos_v6/histograma_IrrDay.png`\n")
    linhas.append("- `graficos_v6/distribuicao_classes_v6.png`\n")
    linhas.append("- `graficos_v6/correlacao_features_v6.png`\n")
...
def main():
    print("\n" + "="*60)
    print("🌿 SIMULAÇÃO AQUACROP v6 — Cold Start ALMMo-0")
    ...
    df_todos_out[colunas_almmo].to_csv('dataset_cold_start_v6.csv', index=False)
    df_todos_out[colunas_almmo + ['irr_mm', 'ano', 'cenario']].to_csv(
        'dataset_cold_start_completo_v6.csv', index=False)
    ...
    print(f"\n{'='*60}")
    print(f"✅ dataset_cold_start_v6.csv → {n_total} linhas | {len(colunas_almmo)} colunas")
    print(f"   Colunas: {colunas_almmo}")
    print(f"   Classes: {n_classes} | Limiares: por percentis reais do IrrDay")
    print(f"   Chuva: {feature_chuva}")
    print(f"\n→ Próximo passo: dataset_cold_start_v6.csv → ALMMo-0 cold_start()")
    print("="*60)
