...
Saídas:
  - dataset_cold_start_v4.csv
  - dataset_cold_start_completo_v3.csv
  - cenario1_otimo_v4.csv / cenario2_deficit_v4.csv / cenario3_excesso_v4.csv
  - graficos_v4/ (distribuicao_classes, correlacao, histograma_IrrDay)
...
CENARIOS = [
    (1, 'cenario1_otimo_v4',   65, 'Irrigação Ótima  (NetIrrSMT=65%)'),
    (2, 'cenario2_deficit_v4', 30, 'Déficit Hídrico  (NetIrrSMT=30%)'),
    (3, 'cenario3_excesso_v4', 90, 'Excesso Hídrico  (NetIrrSMT=90%)'),
]
...
def calcular_limiares_percentis(df_todos):
    """
    v4: limiares calculados APENAS sobre o cenário ótimo (NetIrrSMT=65%).
    Justificativa: o déficit tem IrrDay truncado em 5mm (viés para baixo)
    e o excesso tem IrrDay concentrado acima de 10mm (viés para cima).
    O cenário ótimo é o único que representa decisões agronômicas normais
    com distribuição não truncada. P25/P50/P75 sobre n=534 são estáveis.
    """
    n_total = len(df_todos)

    # Distribuição por cenário — informativa
    print(f"\n{'='*60}")
    print("🔍 ANÁLISE DO IrrDay BRUTO (v4 — limiares pelo cenário ótimo)")
    ...
    for nome_c in df_todos['cenario'].unique():
        ...
        label = nome_c.replace('cenario','C').replace('_v4','')
        if len(irr_pos) > 0:
            print(f"   {label}: n={len(irr_pos)} | "
                  f"min={irr_pos.min():.2f} | "
                  f"P25={irr_pos.quantile(.25):.2f} | "
                  f"P50={irr_pos.quantile(.50):.2f} | "
                  f"P75={irr_pos.quantile(.75):.2f} | "
                  f"max={irr_pos.max():.2f} mm")
        else:
            print(f"   {label}: sem irrigações positivas")

    # Percentis APENAS do cenário ótimo
    nome_otimo = [n for _, n, _, _ in CENARIOS if 'otimo' in n][0]
    irr_otimo  = df_todos[
        (df_todos['cenario'] == nome_otimo) & (df_todos['irr_mm'] > 0)
    ]['irr_mm']
    n_otimo = len(irr_otimo)

    p25 = irr_otimo.quantile(0.25)
    p50 = irr_otimo.quantile(0.50)
    p75 = irr_otimo.quantile(0.75)

    print(f"\n   Percentis do cenário ótimo (n={n_otimo} eventos de irrigação):")
    print(f"   P25 = {p25:.2f} mm  → limiar C1/C2")
    print(f"   P50 = {p50:.2f} mm  → limiar C2/C3")
    print(f"   P75 = {p75:.2f} mm  → limiar C3/C4")
    print(f"   Faixa C1–C4: {irr_otimo.min():.2f} → {irr_otimo.max():.2f} mm")
    print(f"   Amplitude P25→P75: {p75-p25:.2f} mm  (vs {p50*0.63:.2f}mm no v3)")

    limiares = {'p25': p25, 'p50': p50, 'p75': p75}
    n_classes = 5

    # Preview da distribuição resultante
    def rotular_prev(x):
        if x == 0.0:      return 0
        elif x <= p25:    return 1
        elif x <= p50:    return 2
...
def rotular_classe_v4(irr_mm, limiares):
    """v4: limiares P25/P50/P75 do cenário ótimo."""
    if irr_mm == 0.0:          return 0
    elif irr_mm <= limiares['p25']: return 1
    elif irr_mm <= limiares['p50']: return 2
    elif irr_mm <= limiares['p75']: return 3
    else:                           return 4
...
def gerar_graficos(df_final, df_irr_bruto, feature_chuva, n_classes):
    ...
    fig.suptitle(f'Distribuição de Classes por Cenário (v4 — 5 classes, P25/P50/P75 ótimo)',
                 fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(f'{DIR_GRAFICOS}/distribuicao_classes_v4.png', dpi=130)
    plt.close()
    print(f"   📈 Salvo: {DIR_GRAFICOS}/distribuicao_classes_v4.png")
    ...
    ax.set_title('Matriz de Correlação — Dataset v4', fontweight='bold')
    plt.tight_layout()
    plt.savefig(f'{DIR_GRAFICOS}/correlacao_features_v4.png', dpi=130)
    plt.close()
    print(f"   📈 Salvo: {DIR_GRAFICOS}/correlacao_features_v4.png")
    ...
    fig.suptitle('Comparação de Distribuição de Classes — v2 (limiares fixos) vs v4 (P25/P50/P75 ótimo)',
    ...
    fig.suptitle('Comparação de Correlação — v2 (limiares fixos) vs v4 (P25/P50/P75 ótimo)',
                 fontweight='bold')

    for ax, cols, titulo in [(ax1, cols_v2_disp, 'v2 — limiares fixos'),
                              (ax2, cols_v3_disp, f'v4 — P25/P50/P75 ótimo (5 classes)')]:
...
def gerar_relatorio(limiares, feature_chuva, corrs_chuva,
                    ...
                    theta_33, theta_1500, A, B):
    linhas = []
    linhas.append("# Relatório de Correções — Dataset Cold Start v4\n")
    ...
    linhas.append("## Rotulagem — 5 classes por P25/P50/P75 do cenário ótimo\n")
    linhas.append("**Justificativa:** percentis calculados apenas sobre o cenário ótimo (NetIrrSMT=65%) "
                  "porque o déficit tem IrrDay truncado em 5mm e o excesso tem IrrDay deslocado para cima. "
                  "O cenário ótimo é o único com distribuição não truncada do IrrDay.\n\n")
    linhas.append("**Limiares adotados (IrrDay > 0, n=534 eventos do cenário ótimo):**\n")
    linhas.append(f"- Classe 0: IrrDay = 0,0 mm (sem irrigação)\n")
    linhas.append(f"- Classe 1: 0 < IrrDay ≤ {limiares['p25']:.2f} mm (P25)\n")
    linhas.append(f"- Classe 2: {limiares['p25']:.2f} < IrrDay ≤ {limiares['p50']:.2f} mm (P50)\n")
    linhas.append(f"- Classe 3: {limiares['p50']:.2f} < IrrDay ≤ {limiares['p75']:.2f} mm (P75)\n")
    linhas.append(f"- Classe 4: IrrDay > {limiares['p75']:.2f} mm\n")
    linhas.append(f"\nAmplitude P25→P75: {limiares['p75']-limiares['p25']:.2f} mm "
                  f"(vs 4,32 mm no v3 com percentis combinados)\n\n")
    ...
    linhas.append("- `dataset_cold_start_v4.csv` — 5 colunas para ALMMo-0\n")
    linhas.append("- `dataset_cold_start_completo_v3.csv` — com metadados de cenário/ano\n")
    linhas.append("- `graficos_v4/histograma_IrrDay.png`\n")
    linhas.append("- `graficos_v4/distribuicao_classes_v4.png`\n")
    linhas.append("- `graficos_v4/correlacao_features_v4.png`\n")
...
def main():
    print("\n" + "="*60)
    print("🌿 SIMULAÇÃO AQUACROP v4 — Cold Start ALMMo-0")
    ...
    df_todos['classe_irrigacao'] = df_todos['irr_mm'].apply(
        lambda x: rotular_classe_v4(x, limiares))
    ...
    df_todos_out[colunas_almmo].to_csv('dataset_cold_start_v4.csv', index=False)
    ...
    print(f"\n{'='*60}")
    print(f"✅ dataset_cold_start_v4.csv → {n_total} linhas | {len(colunas_almmo)} colunas")
    print(f"   Colunas: {colunas_almmo}")
    print(f"   Classes: {n_classes} | Limiares: por percentis reais do IrrDay")
    print(f"   Chuva: {feature_chuva}")
    print(f"\n→ Próximo passo: dataset_cold_start_v4.csv → ALMMo-0 cold_start()")
    print("="*60)
